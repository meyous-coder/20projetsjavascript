const affichageScore = document.querySelector("h2");
  ...
  for (let i = 0; i < nbRow; i++) {
    for (let j = 0; j < nbCol; j++) {
      if (briques[i][j].statut === 1) {
        ...
        ctx.beginPath();
        ctx.fillRect(briqueX, briqueY, largeurBrique, hauteurBrique);
        ctx.fillStyle = "#333";
        ctx.fill();
        ctx.closePath();
      }
  ...
  if (fin === false) {
    ...
    if (y + vitesseY > canvas.height - rayonBalle) {
      ...
      if (x > barreX && x < barreX + barreWidth) {
        ...
        fin = true;
        affichageScore.innerHTML = `<span style="color: white;">🪦 PERDU 🪦</span> <br> Cliquez sur le casse-briques pour recommencer 🕹`;
      }
    }
